...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const chatMainPlaceholder = document.querySelector('.chat-main .placeholder-text');
    const chatHeaderMain = document.querySelector('.chat-main .chat-header-main');
    const chatMembersContainer = document.querySelector('.chat-main .chat-members');
    const chatMessagesArea = document.querySelector('.chat-main .chat-messages-area');
    const chatMessageInputForm = document.querySelector('.chat-main .message-input-form');
    ...
    function initializeChatSocket() {
        ...
        chatSocket.on('authError', (data) => { console.error('Chat authentication error:', data.message);});
        chatSocket.on('chatError', (data) => { console.error('Chat Error:', data.message);});
        ...
        chatSocket.on('userStatusChanged', ({ phpStudentId, status, userId }) => {
            ...
            if (isMessagesPage()) {
                updateUserStatusInUI(status, userId);
            }
        });

        chatSocket.on('notification', ({ message, chatRoomId, chatRoomName, isGroup }) => {
            console.log('Received notification:', message);
            // Анімація і індикатор непрочитаних сповіщень
            bellIcon.classList.add("shake-animation");
            badge.classList.remove("hidden");
            badge.textContent = parseInt(badge.textContent) + 1;
            setTimeout(() => { bellIcon.classList.remove("shake-animation");}, 800);
            // Додати сповіщення до списку непрочитаних
    ...
    function showChatArea(show) {
        if (chatMainPlaceholder) chatMainPlaceholder.style.display = show ? 'none' : 'flex';
        if (chatHeaderMain) chatHeaderMain.style.display = show ? 'flex' : 'none';
        if (chatMembersContainer) chatMembersContainer.style.display = show ? 'flex' : 'none';
        if (chatMessagesArea) chatMessagesArea.style.display = show ? 'flex' : 'none';
        if (chatMessageInputForm) chatMessageInputForm.style.display = show ? 'flex' : 'none';
    }
    ...
    function renderChatList(chatRooms) {
        ...
        chatRooms.forEach(room => {
            ...
            if (activeChatRoomId === room._id) li.classList.add('active');
            ...
            li.addEventListener('click', () => {
                ...
                if(headerDiv) headerDiv.innerHTML = `<h3>${escapeHTML(displayName)}</h3>`;
    ...
    function renderChatMembers(participants) {
        ...
        participants.forEach(p => {
            ...
            avatarSpan.setAttribute('data-user-mongo-id', p._id);
            if (p.phpStudentId === currentUser.id) {
                avatarSpan.style.border = `3px solid var(--color-accent-primary)`;
    ...
    function appendMessageToChat(message, shouldScroll = true) {
        ...
        if (shouldScroll) scrollToBottom(scrollContainer);
    }
    ...
    function updateUserStatusInUI(status, userId) {
        if (activeChatRoomId) {
            ...
            if (memberAvatar) {
                ...
                memberAvatar.classList.add(status);
    ...
    function addNotificationToList(message, chatRoomId, chatRoomName, isGroup) {
        ...
        li.innerHTML = `
            ...
            <div class="message-text">
                <strong>${escapeHTML(message.senderId.firstName)} ${isGroup ? `in ${truncateText(chatRoomName, 10)}` : ""}</strong>
        ...
        `;
        li.addEventListener('click', () => { window.location.href = `messages.html?chatId=${chatRoomId}`;});
    ...
    async function fetchAllStudentsForChatModal() {
        ...
        try {
            const response = await fetch('api.php/students?limit=1000', { credentials: 'include' });
    ...
    function checkUrlForChatIdAndActivate() {
        if (isMessagesPage() && currentUser && currentUser.id) {
    ...
    if (isMainPage) {
        ...
        function removeStudentFromServer(id) {
            ...
            fetch(`api.php/students/${id}`, { method: 'DELETE', credentials: 'include' })
                .then(response => response.json())
                .then(data => {
                    if (data.success) fetchStudents(currentPage);
                    else alert('Error deleting student: ' + (data.error || 'Unknown error'));
                })
